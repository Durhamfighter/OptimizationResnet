...
class BasicBlock(nn.Module):
    ...
    def prune_now(self,layer_to_prune,filters_to_keep):
        
        if self.conv1==layer_to_prune:
            self.conv1 = prune_layer(layer_to_prune,filters_to_keep)
            self.bn1 = adjust_batch_layer(self.bn1,filters_to_keep)
        else:
            self.conv2 = prune_layer(layer_to_prune,filters_to_keep)
            self.bn2 = adjust_batch_layer(self.bn2,filters_to_keep)


    def prune_next(self,next_layer_to_prune,filters_to_keep):
        
        if self.conv1==next_layer_to_prune:
            self.conv1 = adjust_next_layer(next_layer_to_prune,filters_to_keep)
        else:
            self.conv2 = adjust_next_layer(next_layer_to_prune,filters_to_keep)

        
    def adjust_downsample(self,layer_to_prune,filters_to_keep):
        if self.downsample is None and self.conv1.in_channels != self.conv2.out_channels:
            # add kernel size =1 
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None
        ...
        #fuse model때는 relu를 이름별로 구분해야함 qat에서는!
